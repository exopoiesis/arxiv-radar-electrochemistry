    ...
    for topic, query in queries.items():
        ...
        for r in results:
            ...
            if pid in pid_to_month:
                ...
                continue
            relevant = is_relevant_paper(rec.get("abstract", ""))
            wl_note = None
            if not relevant:
            ...
            added += 1
        LOG.info(f"  {topic}: fetched={len(results)} new={added} (whitelist={whitelist_admitted}) merged={topic_merges} relevance_skipped={relevance_skipped}")
        new_total += added
