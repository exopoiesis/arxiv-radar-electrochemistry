#!/usr/bin/env python3
"""Apply the config-driven relevance filter to an existing corpus locally.

Instead of re-fetching from arXiv, use the data already under data/. Each
paper's abstract is scanned with `relevance_filter` from config.yaml;
non-matching papers are dropped. Output is written to a separate folder so the
...
import data_io
from data_io import is_relevant_paper  # noqa: F401  (re-export for tests/CLI)
...
def run(out_dir):
    ...
    for month, papers in sorted(by_month.items()):
        ...
        for pid, rec in papers.items():
            if is_relevant_paper(rec.get("abstract", "")):
                kept[pid] = rec
