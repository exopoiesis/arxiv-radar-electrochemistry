...
# -- Config-driven relevance filter -------------------------------------------
# The template keeps filtering domain-agnostic. Tune `relevance_filter` in
# config.yaml instead of hard-coding project-specific regexes here.
CONFIG_PATH = ROOT / "config.yaml"


def load_project_meta(config_path=CONFIG_PATH):
    """Return optional display metadata from config.yaml."""
...
def _load_relevance_filter(config_path=CONFIG_PATH):
    if not Path(config_path).exists():
        return {"enabled": False}
    with open(config_path, encoding="utf-8") as f:
        cfg = yaml.safe_load(f) or {}
    return cfg.get("relevance_filter", {}) or {"enabled": False}


def is_relevant_paper(abstract):
    """True when an abstract passes the config-driven relevance filter.

    If `relevance_filter.enabled` is false, every non-empty abstract passes.
    Otherwise a paper passes when it matches any direct domain pattern, or when
    it matches both a method pattern and a domain/context pattern.
    """
    if not abstract:
        return False
    ...
    if methods and domains:
        return bool(methods.search(abstract) and domains.search(abstract))
    if domains:
        return bool(domains.search(abstract))
    return True


# Short alias for projects that want a domain-specific wrapper later.
is_domain_paper = is_relevant_paper
    ...
    Each filter list becomes an OR-joined query. Per-filter rules:
      ...
      - Prefix 'RAW:' → expression passed through, wrapped in parens.
        Lets you write compound queries:
            RAW:"Digital Twin" AND ("your domain" OR "domain context")
