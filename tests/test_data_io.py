...
def test_is_relevant_paper_positive():
    """Direct domain terms and method+domain combinations trigger the filter."""
    import data_io
    assert data_io.is_relevant_paper("This paper studies mackinawite iron sulfide surfaces.")
    assert data_io.is_relevant_paper("Density functional theory for iron sulfide surfaces.")
    assert data_io.is_relevant_paper("Electrochemical CO2 reduction on sulfide catalysts.")


def test_is_relevant_paper_negative():
    """Generic AI text without domain context does not match."""
    import data_io
    assert not data_io.is_relevant_paper("LLM for code generation.")
    assert not data_io.is_relevant_paper("Graph neural networks for image classification.")
    assert not data_io.is_relevant_paper("Robot navigation in unknown environments.")
    assert not data_io.is_relevant_paper("")
...
def test_load_keyword_queries_raw_passthrough(tmp_path):
    ...
    assert '"AI for Your Domain"' in q
    assert '("Digital Twin" AND ("your domain" OR "domain context"))' in q
    assert " OR " in q
